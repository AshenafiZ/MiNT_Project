...
const addKpi = async (req, res) => {
  try {
    ...
    if (!title ) {
      return res.status(400).json({ message: "Title is required for KPI." });
    }
...
const deleteKpa = async (req, res) => {
  try {
    const kpa = await Kpa.findByPk(req.params.id); 
    if (!kpa) {
      return res.status(404).json({ message: "KPA not found." });
    }
    await kpa.destroy(); 
    res.status(200).json({ message: "kpa deleted successfully." });
  } catch (error) {
    res.status(500).json({ message: "Error deleting kpa." });
  }
};
...
module.exports = { getAllGoals, createGoal, addKpa, addKpi, updateGoal, deleteGoal, deleteKpa, deleteKpi };
